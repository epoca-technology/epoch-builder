...
# Epoch Assets' Paths
# The paths for all the directories within the epoch directory which are used
# to read and write results.
class IEpochAssetsPath(TypedDict):
    # Backtest Configurations
    backtest_configurations: str
    ...
    # Configurations generated by Hyperparams in order to train Models
...
class IEpochConfig(TypedDict):
    ...
    # The range of the Epoch. These values are used for calculate the training evaluation 
    # range (1 - train_split)
    start: int
    end: int
    ...
    # 3) Backtest the Classification Selection as well as the Consensus Models
